...
extern "C" void test_prg(const uint8_t *key, int pt, uint8_t *ct1, uint8_t *ct2, int num_blocks, int tb_size)
{
  ...
  checkCudaErrors(cudaMalloc(&d_key, mem_size));
  checkCudaErrors(cudaMalloc(&d_ct1, mem_size));
  checkCudaErrors(cudaMalloc(&d_ct2, mem_size));

  cudaMemcpy(d_key, key, mem_size, cudaMemcpyHostToDevice);

  apply_aes_prg_wrapper<<<num_blocks, tb_size>>>((const uint8_t *)d_key, pt, (uint8_t *)d_ct1, (uint8_t *)d_ct2, n);

  checkCudaErrors(cudaDeviceSynchronize());
  checkCudaErrors(cudaMemcpy(ct1, d_ct1, mem_size, cudaMemcpyDeviceToHost));
  checkCudaErrors(cudaMemcpy(ct2, d_ct2, mem_size, cudaMemcpyDeviceToHost));
}
