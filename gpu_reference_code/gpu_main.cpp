...
int main(int argc, char *argv[]) {
    ...
    auto layer2 = ReluSignExtendLayer(bl - scale, bl, config.N * 14 * 14 * 64);
    // auto layer0 = FCLayer(bl, bl, config.N, 10, 64, TruncateType::LocalLRS, TruncateType::StochasticTruncate, false);

    u64 insize = config.N * 32 * 32 * 3;
    u64 outsize = config.N * 14 * 14 * 64;
    // u64 insize = config.N * 64;
    // u64 outsize = config.N * 10;
            ...
            #pragma omp parallel 
            {
                #pragma omp sections 
                {
                    ...
                    #pragma omp section 
                    {
                        ...
                        // }
                        // do batches better
                        auto res = maskInput(insize, bl, config.party-2, peer, data, layer0.convKey.I/*matmulKey.A*/, NULL);
                        auto d_I = res.first;
