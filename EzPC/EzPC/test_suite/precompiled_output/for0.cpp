...
share* summ = put_cons32_gate(acirc,  (int32_t)0);

int32_t N =  (int32_t)100;

auto arr = make_vector<share*>( (int32_t)100);
for (uint32_t i =  (int32_t)0; i <  (int32_t)100; i++){
arr[i] = put_cons32_gate(acirc, i);
}
for (uint32_t i =  (int32_t)0; i <  (int32_t)100; i++){
/* Temporary variable for sub-expression on source location: (173,16-173,22) */
share* __tac_var78 = arr[i];
summ = acirc->PutADDGate(summ, __tac_var78);
}
add_print_msg_to_output_queue(out_q, "Value of summ:", CLIENT, cout);
add_to_output_queue(out_q, acirc->PutOUTGate(summ, CLIENT), CLIENT, cout);
party->ExecCircuit();
