...
/* Variable to read the clear value corresponding to the input variable w at (167,1-167,28) */
uint32_t __tmp_in_w;
if ((role == SERVER)) {
cin >> __tmp_in_w;
}
...
/* Variable to read the clear value corresponding to the input variable x at (168,1-168,28) */
uint32_t __tmp_in_x;
if ((role == SERVER)) {
cin >> __tmp_in_x;
}
...
/* Variable to read the clear value corresponding to the input variable z at (169,1-169,32) */
uint32_t __tmp_in_z;
...
/* Variable to read the clear value corresponding to the input variable a at (170,1-170,37) */
uint32_t __tmp_in_a;
for (uint32_t i0 =  (uint32_t)0; i0 <  (int32_t)10; i0++){
...
/* Variable to read the clear value corresponding to the input variable b at (171,1-171,26) */
uint32_t __tmp_in_b;
if ((role == SERVER)) {
cin >> __tmp_in_b;
}
b = (role == SERVER) ? ycirc->PutINGate(__tmp_in_b, 1, SERVER) : ycirc->PutDummyINGate(1);
add_print_msg_to_output_queue(out_q, "Value of w:", SERVER, cout);
add_to_output_queue(out_q, acirc->PutOUTGate(w, SERVER), SERVER, cout);
add_print_msg_to_output_queue(out_q, "Value of x:", ALL, cout);
add_to_output_queue(out_q, ycirc->PutOUTGate(x, ALL), ALL, cout);
add_print_msg_to_output_queue(out_q, "Value of z:", SERVER, cout);
for (uint32_t i0 =  (uint32_t)0; i0 <  (int32_t)10; i0++){
add_to_output_queue(out_q, acirc->PutOUTGate(z[i0], SERVER), SERVER, cout);
}
add_print_msg_to_output_queue(out_q, "Value of a:", CLIENT, cout);
for (uint32_t i0 =  (uint32_t)0; i0 <  (int32_t)10; i0++){
for (uint32_t i1 =  (uint32_t)0; i1 <  (int32_t)100; i1++){
add_to_output_queue(out_q, acirc->PutOUTGate(a[i0][i1], CLIENT), CLIENT, cout);
}
}
add_print_msg_to_output_queue(out_q, "Value of b:", ALL, cout);
add_to_output_queue(out_q, ycirc->PutOUTGate(b, ALL), ALL, cout);
party->ExecCircuit();
