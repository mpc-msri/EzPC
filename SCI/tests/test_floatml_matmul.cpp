...
auto inp1 = make_vector_float(ALICE, m, n) ;
for (int i = 0 ; i < m ; i++) {
	for (int j = 0 ; j < n ; j++) {
		if (__party == ALICE)
...
float* inp2_tmp = new float[1] ;
auto inp2 = make_vector_float(BOB, n, p) ;
for (int i = 0 ; i < n ; i++) {
	for (int j = 0 ; j < p ; j++) {
		if (__party == BOB)
			inp2_tmp[0] = static_cast <float> (rand()) / static_cast <float> (RAND_MAX) ;
		inp2[i][j] = __fp_op->input<float>(BOB, 1, inp2_tmp, m_bits, e_bits) ;
	}
}
...
auto out = make_vector_float(ALICE, m, p) ;

MatMul(m, n, p, inp1, inp2, out) ;
