...
# Script accepts 1 argument: path to fptraining
...
# Clone fptraining
echo -e "\${bg_green}Cloning fptraining repository\${clear}"
git clone git@github.com:kanav99/fptraining.git
cd fptraining
git switch onnx-sytorch
cd ..
wait

fptraining="\$current_dir/fptraining"
...
python \$fptraining/OnnxBridge/main.py --path $File_NAME --backend $BACKEND --scale $SCALE --bitlength $BITLENGTH 
wait
\$fptraining/OnnxBridge/LLAMA/compile_llama.sh "${Model_Name}_${BACKEND}_${SCALE}.cpp"
...
python -m http.server 8000 --bind $SERVER_IP &

# Get the process ID of the Python process
PID=\$!

# Kill the Python process after the first connection is completed
...
echo -e "\${bg_green}Client has successfully completed the inference.\${clear} \${bg_red}Killing the server.\${clear}"
kill \$PID
echo -e "\${bg_red}Killed process with PID \$PID.\${clear}"
...
while true; do
    ...
    STATUS=\$(curl -s -w '%{http_code}' "http://$SERVER_IP:\$SERVER_PORT/client_$Model_Name.zip" --output client_lenet.zip)
...
wait
python \$fptraining/OnnxBridge/helper/convert_np_to_float_inp.py --inp \$Image_Name.npy --out \$Image_Name.inp
