...
def build_parser():
    ps = ArgumentParser()
    ps.add_argument(
        "--in", dest="input", help="input file", metavar="INPUT", required=True
    )
    ps.add_argument(
        "--saveTFMetadata",
        dest="saveTFMetadata",
        type=bool,
        help="bool to indicate if to save metadata",
        required=False,
    )
    ps.add_argument(
        "--saveImgAndWtData",
        dest="saveImgAndWtData",
        type=bool,
        help="bool to indicate if to save img and model weights",
        required=False,
    )
    ps.add_argument(
        "--savePreTrainedWeightsFloat",
        dest="savePreTrainedWeightsFloat",
        type=bool,
        help="bool to indicate if to save model weights float",
        required=False,
    )
    ps.add_argument(
        "--savePreTrainedWeightsInt",
        dest="savePreTrainedWeightsInt",
        type=bool,
        help="bool to indicate if to save model weights int",
        required=False,
    )
    ps.add_argument(
        "--saveImgAndWeightsSeparately",
        dest="saveImgAndWeightsSeparately",
        type=bool,
        help="bool to indicate if to save image and model weights int separately",
        required=False,
    )
    ps.add_argument(
        "--scalingFac",
        ...
        if options.saveTFMetadata:
            ...
            if options.savePreTrainedWeightsInt:
                DumpTFMtData.dumpTrainedWeightsInt(
                    sess,
                    all_weights,
                    "model_weights_scale_{}.inp".format(options.scalingFac),
                    options.scalingFac,
                    "w",
            ...
            if options.saveImgAndWtData:
                DumpTFMtData.dumpImgAndWeightsDataSeparate(
                    ...
                    imageData,
                    all_weights,
                    "model_input_scale_{}.inp".format(options.scalingFac),
                    "model_weights_scale_{}.inp".format(options.scalingFac),
                    options.scalingFac,
                    alreadyEvaluated=True,
                )
