...
**Output:**
After connecting to the session with ```tmux a -t ResNet``` you should see output similar to the following after the computation is complete. Number will vary based on the specs of your machine.
...
To run the network in **CPP mode (1PC-debug-non-secure)**, open a terminals and do the following:
